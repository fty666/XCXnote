    ...
    <el-table
      ...
      <el-table-column fixed="right" label="操作" align="center" width="340">
        <template slot-scope="scope">
          <el-button type="warning"  @click="select(scope.$index,scope.row)">分配</el-button>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      update(){
        this.$http //列表加载
          .post(this.url + "adminInternal/addItemsByXiaoShou", {
      ...
      getList(){
        this.$http //列表加载
          .post(this.url + "adminInternal/selectItemsAllocationByItemsNumber", {
            page:this.page,
